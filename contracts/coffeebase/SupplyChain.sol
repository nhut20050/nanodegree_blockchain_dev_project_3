...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  // Check an item is existed or not
  ...
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "You do not have enought money to paid for the item."); 
    _;
  }
  ...
  ) public onlyFarmer  doesNotExist(_upc) {
  ...
  function buyItem(uint _upc) public payable onlyDistributor forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) {
    ...
    // address payable originFarmerAddressPayable = address(uint160(items[_upc].originFarmerID));
    // originFarmerAddressPayable.transfer(msg.value);
    (bool successStatus,) = items[_upc].ownerID.call.value(items[_upc].productPrice)("");
    require(successStatus, "Failed to transfer money to previous owner.");
